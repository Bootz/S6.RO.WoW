...
<<<<<<< HEAD:src/server/scripts/Northrend/icecrown.cpp
};
...
=======
>>>>>>> tc:src/server/scripts/Northrend/icecrown.cpp
};
...
=======
	...
	NPC_FACTION_VAILIANT_TEXT_SAY_START_1 	= -1850004,//	Tenez-vous prï¿½t !
	NPC_FACTION_VAILIANT_TEXT_SAY_START_2 	= -1850005,//	Que le combat commence !
	NPC_FACTION_VAILIANT_TEXT_SAY_START_3 	= -1850006,//	Prï¿½parez-vous !
	NPC_ARGENT_VAILIANT_TEXT_SAY_START 		= -1850007,//	Vous pensez avoir la vaillance en vous ? Nous verrons.
	NPC_ARGENT_VAILIANT_TEXT_SAY_WIN 		= -1850008,//	Impressionnante dï¿½monstration. Je pense que vous ï¿½tes tout ï¿½ fait en mesure de rejoindre les rangs des vaillants.
	NPC_ARGENT_VAILIANT_TEXT_SAY_LOOSE 		= -1850009,//	J'ai gagnï¿½. Vous aurez sans doute plus de chance la prochaine fois.
	NPC_FACTION_VAILIANT_TEXT_SAY_WIN_1 	= -1850010,//	Je suis vaincue. Joli combat !
	NPC_FACTION_VAILIANT_TEXT_SAY_WIN_2 	= -1850011,//	On dirait que j'ai sous-estimï¿½ vos compï¿½tences. Bien jouï¿½.
	NPC_FACTION_VAILIANT_TEXT_SAY_LOOSE 	= -1850012,//	J'ai gagnï¿½. Vous aurez sans doute plus de chance la prochaine fois.
};
...
#define GOSSIP_HELLO_VALIS_1 "Montrez-moi comment m'entraÃ®ner sur une cible ï¿½ distance."
    ...
    bool GossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
    {
    	switch (uiAction)
    	{
    		case GOSSIP_ACTION_INFO_DEF+1:
    			...
    			pPlayer->CLOSE_GOSSIP_MENU();//Ferme la fenetre du gossip cotï¿½ client
    		break;
    		case GOSSIP_ACTION_INFO_DEF+2:
    			//Raconte un blabla
...
<<<<<<< HEAD:src/server/scripts/Northrend/icecrown.cpp
	...
	newscript = new Script;
    newscript->Name = "npc_vendor_argent_tournament";
    newscript->pGossipHello = &GossipHello_npc_vendor_argent_tournament;
    newscript->RegisterSelf();

	newscript = new Script;
    newscript->Name = "npc_quest_givers_argent_tournament";
    newscript->pGossipHello = &GossipHello_quest_givers_argent_tournament;
    newscript->RegisterSelf();

	newscript = new Script;
    newscript->Name = "npc_training_dummy_argent";
    newscript->GetAI = &GetAI_npc_training_dummy_argent;
    newscript->RegisterSelf();
	
	newscript = new Script;
    newscript->Name = "npc_quest_givers_for_crusaders";
    newscript->pGossipHello = &GossipHello_npc_quest_givers_for_crusaders;
    newscript->RegisterSelf();

	newscript = new Script;
    newscript->Name = "npc_justicar_mariel_trueheart";
    newscript->pGossipHello = &GossipHello_npc_justicar_mariel_trueheart;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_crusader_rhydalla";
    newscript->pGossipHello = &GossipHello_npc_crusader_rhydalla;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_eadric_the_pure";
    newscript->pGossipHello = &GossipHello_npc_eadric_the_pure;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_crok_scourgebane";
    newscript->pGossipHello = &GossipHello_npc_crok_scourgebane;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_valis_windchaser";
    newscript->pGossipHello = &GossipHello_npc_valis_windchaser;
    newscript->pGossipSelect = &GossipSelect_npc_valis_windchaser;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_rugan_steelbelly";
    newscript->pGossipHello = &GossipHello_npc_rugan_steelbelly;
    newscript->pGossipSelect = &GossipSelect_npc_rugan_steelbelly;
    newscript->RegisterSelf();
    
    newscript = new Script;
    newscript->Name = "npc_jeran_lockwood";
    newscript->pGossipHello = &GossipHello_npc_jeran_lockwood;
    newscript->pGossipSelect = &GossipSelect_npc_jeran_lockwood;
    newscript->RegisterSelf();
