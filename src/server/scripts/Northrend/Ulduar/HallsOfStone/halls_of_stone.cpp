        ...
        void SetInCombat()
        {
            ...
            for(Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                if (Player* i_pl = i->getSource())
                    if (i_pl->isAlive() && me->GetDistance(i_pl) <= 100)
                    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (bMarnakActivated)
            {
                if (uiMarnakEncounterTimer <= diff)
                {
                    ...
                    uiMarnakEncounterTimer = 15*IN_MILLISECONDS + rand()%5*IN_MILLISECONDS;
            ...
            if (bAbedneumActivated)
            {
                if (uiAbedneumEncounterTimer <= diff)
                {
                    ...
                    uiAbedneumEncounterTimer = 17500 + rand()%5*IN_MILLISECONDS;
