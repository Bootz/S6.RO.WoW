...
public:
    ...
    struct npc_lake_frogAI : public FollowerAI // FollowerAI:Permet au npc de suivre une cible
    {
        ...
        {
    		if(following) //Si la grenouille a dï¿½ja recu un /bisou il ne se passe rien
    				return;
    ...
    struct npc_maiden_of_drak_marAI : public ScriptedAI
    {
    	...
    	npc_maiden_of_drak_marAI(Creature *c) : ScriptedAI(c)
    	{
    		...
    		if(GameObject* go = me->SummonGameObject(MAIDEN_OF_DRAK_MAR_GOB_01,4602.977f,-1600.141f,156.7834f,0.7504916f,0,0,0,0,0))
    			firstGobGuid = go->GetGUID(); //Pop du nï¿½nuphar
    	}
    				...
    				switch(phase)
    				{
    					...
    					case 5:
    						if(GameObject* go = GameObject::GetGameObject(*me,firstGobGuid))
    							go->RemoveFromWorld();// Dï¿½pop du nï¿½nuphar
    						if(GameObject* go = GameObject::GetGameObject(*me,secondGobGuid))
    							go->RemoveFromWorld();// Dï¿½pop de la lame 
...
#define GOSSIP_HELLO_MAIDEN "Ravi d'avoir pu aider, madame. Il paraÃ®t que vous Ãªtiez autrefois la gardienne d'une ÃªpÃªe lÃªgendaire. Sauriez-vous oÃ¹ je pourrais la trouver ?"
#define SPELL_SUMMON_ASHWOOD_BRAND 62554
bool GossipHello_npc_maiden_of_ashwood_lake(Player* pPlayer, Creature* pCreature)
{
	if(!pPlayer->HasItemCount(44981,1,true))
	{
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_HELLO_MAIDEN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
bool GossipSelect_npc_maiden_of_ashwood_lake(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
	switch(uiAction)
	{
		case GOSSIP_ACTION_INFO_DEF+1:
...
CreatureAI* GetAI_npc_maiden_of_drak_mar(Creature* pCreature)
{
	return new npc_maiden_of_drak_marAI(pCreature);
}
...
{
<<<<<<< HEAD:src/server/scripts/Northrend/grizzly_hills.cpp
    Script* newscript;
	...
	newscript = new Script;
    newscript->Name = "npc_maiden_of_ashwood_lake";
    newscript->pGossipHello = &GossipHello_npc_maiden_of_ashwood_lake;
    newscript->pGossipSelect = &GossipSelect_npc_maiden_of_ashwood_lake;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_maiden_of_drak_mar";
    newscript->GetAI = &GetAI_npc_maiden_of_drak_mar;
    newscript->RegisterSelf();
...
>>>>>>> tc:src/server/scripts/Northrend/grizzly_hills.cpp
}
